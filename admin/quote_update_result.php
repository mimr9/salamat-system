...
<?php
					$test_writer = test_input($_POST['writer']);
					$test_content = test_input($_POST['content']);

					$id = (int) $_POST['id'];
					if (empty($test_writer) && $id == 0) {
						echo '<br>';
						...
						$query = 'update quotes set writer = :writer, content = :content where id = :id';
						$statement = $db->prepare($query);
						$statement->bindValue('writer', $test_writer);
						...
						if ($statement->execute()) {
							$message = '<div class="jumbotron jumbotron-fluid">
							  <div class="container" style="font-family: vazir, serif; text-align: right;">
								...
								<a href="/salamat/admin/quotes.php">بازگشت</a>
							  </div>
							</div>';
							echo $message;
						} else {
