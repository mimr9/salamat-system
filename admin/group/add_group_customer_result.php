						...
						if(!empty($_POST)) {
							if(isset($_POST['checkbox'])){
								foreach($_POST['checkbox'] as $id => $keyt) {
										$query = 'insert into '. $test_name_en .' (user_id, customer_id) values ( '. $user_id .', ' . $id . ' )';
								...
								$message = '<div class="jumbotron jumbotron-fluid">
								  <div class="container" style="font-family: vazir, serif; text-align: right;">
								    ...
								    <p class="lead">شما با موفقیت مراجعه‌کنندگان را به گروه اضافه کردید!</p>
									<a href="/salamat/admin/group/add_group_customer_list.php">بازگشت</a>
								  </div>
								</div>';
								echo $message;
							} else {
