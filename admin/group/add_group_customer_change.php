...
<?php
						...
						$query = 'select id, concat(firstname, " ", lastname) as name, national, mobile, address from customer where id not in ( select customer_id from '. $test_name_en .' )';
						$statement = $db->prepare($query);
						if($statement->execute()) {
							$rows = $statement->fetchAll();
								//start table head
								echo '
							<form method="post" action="/salamat/admin/group/add_group_customer_result.php">
								...
								<table class="table table-striped table-bordered vazir centerize">
								  <thead>
								    <tr>
								      ...
								      <th scope="col"><input type="submit" class="btn btn-primary" value="افزودن"></th>
								    </tr>
								  </thead>
								  <tbody>
								';
...
<script>
	...
	$("#side34").addClass("nav-link");
	$("#side34").addClass("disabled");
</script>
